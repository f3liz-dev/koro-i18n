...
import { Project, ProjectSettings, TranslationFile } from '../types/Project.js';
import { TranslationConfig } from '../../shared/types/config.js';
...
export class ProjectService {
  ...
  async discoverProjects(): Promise<Project[]> {
    return telemetry.traced('project.discover', async () => {
      try {
        ...
        telemetry.recordProject('discover', undefined, true);
        return projects;
      } catch (error) {
        telemetry.recordProject('discover', undefined, false);
  ...
  async getProject(projectId: string): Promise<Project | null> {
    return telemetry.traced('project.get', async () => {
      const cachedProject = this.projects.get(projectId);
      if (cachedProject) {
        telemetry.recordProject('get', projectId, true);
      ...
      if (parts.length >= 3) {
        ...
        try {
          ...
          telemetry.recordProject('get', projectId, false);
          return null;
        }
      }
  ...
  async validateProjectAccess(projectId: string): Promise<boolean> {
    return telemetry.traced('project.validate_access', async () => {
      const project = await this.getProject(projectId);
      if (!project) {
        telemetry.recordProject('validate_access', projectId, false);
        return false;
      }
      ...
      try {
        ...
        const hasAccess = access.hasAccess && access.permissions.write;
        telemetry.recordProject('validate_access', projectId, hasAccess);
        return hasAccess;
      } catch (error) {
        telemetry.recordProject('validate_access', projectId, false);
        return false;
